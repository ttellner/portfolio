...
with st.sidebar:
    ...
    # 🔹 NEW SECTION: Dynamic sidebar navigation
    st.header("📂 Project Navigation")
    project_dir = "pages"  # same directory your projects live in

    if os.path.exists(project_dir):
        for file in sorted(os.listdir(project_dir)):
            if file.endswith(".py"):
                display_name = file.replace(".py", "").replace("_", " ").title()
                if st.button(f"▶ {display_name}", key=file):
                    st.query_params.update({"project": file})
    else:
        st.caption("No project pages found.")
